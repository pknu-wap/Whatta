...
const LUNAR_HOLIDAYS_OFFSETS: Record<number, {
    ...
    2024: {
      설날: [{ month: 1, day: 9 }, { month: 1, day: 10 }, { month: 1, day: 11 }],
      추석: [{ month: 8, day: 16 }, { month: 8, day: 17 }, { month: 8, day: 18 }],
    ...
    2026: {
      설날: [{ month: 1, day: 16 }, { month: 1, day: 17 }, { month: 1, day: 18 }],
      추석: [{ month: 8, day: 24 }, { month: 8, day: 25 }, { month: 8, day: 26 }],
      부처님오신날: { month: 4, day: 24 },
      대체휴일: [
        { month: 2, day: 2 },
        { month: 4, day: 25 },
...
const INITIAL_DUMMY_SCHEDULES: ScheduleData[] = [
  // 단일 일정
  { id: 's1', name: '외할머니댁', date: '2025-10-06', isRecurring: false, isTask: false, labelId: '1', isCompleted: false, },
  { id: 't4', name: '코드제출', date: '2025-10-30', isRecurring: false, isTask: true, labelId: '3', isCompleted: false, },
  { id: 's_wap', name: 'WAP회의', date: '2025-10-07', isRecurring: false, isTask: false, labelId: '2', isCompleted: false, },
  { id: 't_emotion', name: '감성공학 과제제출', date: '2025-10-16', isRecurring: false, isTask: true, labelId: '3', isCompleted: false, },
  // 멀티데이 일정
  { id: 's_midterm', name: '중간고사', date: '2025-10-21', isRecurring: false, isTask: false, labelId: '4', isCompleted: false, multiDayStart: '2025-10-21', multiDayEnd: '2025-10-24' },
  
  // Task Summary 테스트를 위한 Task 2개 이상
  { id: 't_report', name: '레포트쓰기', date: '2025-10-13', isRecurring: false, isTask: true, labelId: '3', isCompleted: false, },
  { id: 't_research', name: '자료조사', date: '2025-10-13', isRecurring: false, isTask: true, labelId: '3', isCompleted: false, },
  
  // 기타 단일 일정
  { id: 's_wap_28', name: 'WAP회의', date: '2025-10-28', isRecurring: false, isTask: false, labelId: '2', isCompleted: false, },
  { id: 's_h_friends', name: '고등학교친구들', date: '2025-10-12', isRecurring: false, isTask: false, labelId: '1', isCompleted: false, },
  { id: 's_c_friends', name: '과친구들', date: '2025-10-26', isRecurring: false, isTask: false, labelId: '1', isCompleted: false, },
  ...
  { id: 's_tue_3', name: '생활속의 감성공학 (화)', date: '2025-09-02', isRecurring: true, isTask: false, labelId: '5', isCompleted: false, },
  // 수요일 (2025-09-03 시작)
  ...
  { id: 's_thu_2', name: '생활속의 감성공학 (목)', date: '2025-09-04', isRecurring: true, isTask: false, labelId: '5', isCompleted: false, },
  // 금요일 (2025-09-05 시작)
...
const ScheduleItem: React.FC<ScheduleItemProps> = ({ schedule, onToggleComplete, currentDateISO, isCurrentMonth }) => {
  ...
  const textColorStyle = isRecurring 
    ? S.recurringScheduleText 
    : S.singleScheduleText;   
  ...
  return (
    ...
    >
      <Text style={[S.scheduleText, textColorStyle]} numberOfLines={1} ellipsizeMode="tail">
        {schedule.name}
      </Text>
    </View>
  );
...
const TaskSummaryBox: React.FC<TaskSummaryBoxProps> = ({ count, tasks, onToggleAllComplete, isCurrentMonth }) => {
  const dimmedStyle = !isCurrentMonth ? S.dimmedItem : null; 
...
export default function MonthView() {
  ...
  const [allSchedules, setAllSchedules] = useState<ScheduleData[]>(INITIAL_DUMMY_SCHEDULES);
            ...
            {renderWeeks(calendarDates).map((week, weekIndex) => (
              <View key={`week-${weekIndex}`} style={S.weekRow}>
                {week.map((dateItem: CalendarDateItem, i: number) => {
                  const itemsToRender = getDisplayItems(dateItem.schedules, dateItem.tasks);
                    ...
                    >
                      ...
                      <View style={S.eventArea}>
                        {itemsToRender.map((item) => {
                          if (typeof item === 'object' && 'isTaskSummary' in item && (item as TaskSummaryItem).isTaskSummary) {
...
const S = StyleSheet.create({
  ...
  taskSummaryBox: { 
    ...
    paddingLeft: 1, paddingRight: 0, marginBottom: ITEM_MARGIN_VERTICAL,
  },
  moreCountText: {
    fontSize: 8, color: '#333', fontWeight: '400', flex: 1, marginTop: -3, 
    textAlign: 'left', paddingRight: TEXT_HORIZONTAL_PADDING, lineHeight: TASK_BOX_HEIGHT,
  },
  dimmedItem: {
    opacity: 0.3,
  },
