...
useEffect(() => {
  ...
  const handleSwipe = useCallback(
    (dir: 'prev' | 'next') => {
      setYm((prevYM) => {
        const { year: y, monthIndex: m } = parseYM(prevYM)
        const base = new Date(y, m + (dir === 'next' ? 1 : -1), 1)
...
/>
    {/* MonthDetailPopup, EventDetailPopup, TaskDetailPopup, OCR 팝업들... */}
        </ScreenWithSidebar>
      </Animated.View>
    </GestureDetector>
  )
