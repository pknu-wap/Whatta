...
function TaskGroupBox({
  ...
  // ✅ 진동 트리거 함수 (expo-haptics 대신 RN Vibration 사용)
  const triggerHaptic = () => {
  ...
  const longPress = Gesture.LongPress()
    .minDuration(2000)
    .maxDistance(1000) // ✅ 크게 설정해서 움직여도 취소되지 않게
    .shouldCancelWhenOutside(false) // ✅ 영역 밖으로 살짝 나가도 유지
    .onStart(() => {
...
function DraggableTaskBox({
  ...
  // ✅ 시간·요일 이동 드롭 처리
const handleDrop = async (newDateISO: string, newTime: string) => {
  try {
    ...
    // 상위 WeekView 상태 즉시 반영
    onLocalChange?.({ id, dateISO: newDateISO, placementTime: newTime })
  ...
  .onEnd(() => {
    ...
    // ✅ 새 날짜 + 새 시간 함께 전달
    runOnJS(handleDrop)(newDateISO, t)
  })
...
function DraggableFlexalbeEvent({
  id, title, place, startMin, endMin, color,
  dateISO, column, columnsTotal, isPartialOverlap, overlapDepth,
  dayColWidth, weekDates,
  ...
  // ✅ drop 시: movedY + dayOffset(며칠 이동했는지) 반영
  ...
  .onEnd(() => {
    ...
    // 현재 요일 index
    const oldIndex = weekDates.indexOf(dateISO)
    const newIndex = oldIndex + dayOffset

    // ⭐ 요일 칸에 딱 들어맞는 위치로 translateX를 고정
    //    (원래 요일로 돌아가는 현상 방지)
    ...
    // ⛔ translateX.value = 0 을 더 이상 하지 않음!
...
export default function WeekView() {
  ...
  // WeekView에서도 DayView처럼 — 데이터까지 로딩된 후 라이브바로 자동 스크롤
useEffect(() => {
